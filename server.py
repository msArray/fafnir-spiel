...
def seed_trash_at_round_start(
    game: Game, n: int = SEED_TRASH_AT_ROUND_START
) -> List[str]:
    seeded: List[str] = []
    for _ in range(n):
        s = draw_one(game)
...
def log_auction_result(game: Game):
    ...
    print(
        f"│ Round={game.round} Turn={game.turn}  Caretaker={game.caretaker}  Offer={offer}"
    )
...
def start_game_if_ready(room: Room):
    ...
    game = Game(
        players=players, caretaker=caretaker, bag=make_bag(), trash=init_trash()
    )
    deal_initial_hands(game)
    ...
    print(
        f"[{ts()}][SERVER] GAME START room={room.room_id} caretaker={game.caretaker} offer={game.offer}"
    )
...
def start_new_game(room: Room):
    ...
    print(
        f"[{ts()}][SERVER] NEW GAME room={room.room_id} caretaker={game.caretaker} offer={game.offer}"
    )
        ...
        connections.append(
            {
                ...
                "role": "player",
                "connected_for_sec": int(
                    time.time() - room.join_ts_by_sid.get(sid, time.time())
                ),
            }
        )
...
def start_round_end(
    ...
    print(
        f"[{ts()}][SERVER] ROUND_END reason={trigger_reason} trigger_color={trigger_color} rw={rw} caretaker={game.caretaker}"
    )
...
async def disconnect(sid):
    ...
    for room_id, room in list(rooms.items()):
        if sid in room.sids or sid in room.spectators:
            ...
            print(
                f"[{ts()}][SERVER] {name} left room={room_id} role={role} after {dur}s"
            )

            await emit_action(
                room,
                "disconnect",
                {"name": name, "role": role, "room_id": room_id, "dur_s": dur},
            )
            await broadcast_room_notice(
                room, "leave", f"{name} Leave the room. {room_id} ({dur}s)"
            )
...
async def join_room(sid, data: Dict[str, Any]):
    ...
    if is_spectator:
        ...
        await sio.emit(
            "player_assigned",
            {"room_id": room_id, "index": -1, "role": "spectator"},
            to=sid,
        )
        await emit_action(room, "join_spectator", {"name": name, "room_id": room_id})
        await broadcast_room_notice(
            room, "join", f"{name} joined as SPECTATOR in room {room_id}"
        )
        await emit_state(room)
        return
    ...
    print(
        f"[{ts()}][SERVER] join_room sid={sid} room={room_id} idx={idx_now} name={name} sids={len(room.sids)}"
    )

    await sio.emit(
        "player_assigned",
        {"room_id": room_id, "index": idx_now, "role": "player"},
        to=sid,
    )
    await emit_action(
        room, "join_player", {"name": name, "room_id": room_id, "slot": idx_now}
    )
    await broadcast_room_notice(
        room, "join", f"{name} join room {room_id} (slot {idx_now})"
    )
    await emit_state(room)
...
async def submit_bid(sid, data: Dict[str, Any]):
    ...
    if player_index != game.current_bidder:
        await sio.emit(
            "bid_rejected",
            {"reason": "not_your_turn", "message": "Not your turn to bid."},
            to=sid,
        )
        return
    ...
    if not isinstance(stones, list):
        await sio.emit(
            "bid_rejected",
            {"reason": "invalid_payload", "message": "stones It must be list"},
            to=sid,
        )
        return
    ...
    if invalid:
        await sio.emit(
            ...
            {
                "reason": "not_in_hand",
                "message": f"There is a stone that is not in my hand: {invalid}",
            },
            to=sid,
        )
        return
    ...
    print(
        f"[{ts()}][SERVER] submit_bid from P{player_index} {p.name} -> bid={filtered}"
    )
    await emit_action(
        room,
        "submit_bid",
    ...
    if len(game.players) == 2 and all(pp.bid_submitted for pp in game.players):
        ...
        if end:
            ...
            await emit_action(
                room,
                "game_end",
                {"room_id": room_id, **end, "score": [pp.score for pp in game.players]},
            )
            await emit_state(room)
            return
        ...
        if should_force_round_end_by_bag(game):
            start_round_end(
                game, trigger_reason="BAG_LOW", trigger_color=None, last_bid_winner=w
            )
            await emit_action(
                room,
                "round_end_start",
        ...
        if is_trash_limit_reached(game, TRASH_LIMIT):
            ...
            start_round_end(
                game, trigger_reason="TRASH_6", trigger_color=trigger, last_bid_winner=w
            )
            await emit_action(
                room,
                "round_end_start",
            ...
            if w is None:
                game.status = (
                    "RESULT: No one bid. Offer->trash, both -1. Press OK/Next."
                )
            else:
...
async def proceed_phase(sid, data: Dict[str, Any]):
    ...
    if game.phase == "ROUND_END":
        ...
        await emit_action(
            room,
            "phase_change",
            {"room_id": room_id, "to": "BIDDING", "from": "ROUND_END"},
        )
        await emit_state(room)
        return
    ...
    if not game.offer:
        ...
        if not ok or should_force_round_end_by_bag(game):
            start_round_end(
                game, trigger_reason="BAG_LOW", trigger_color=None, last_bid_winner=None
            )
            await emit_action(
                room,
                "round_end_start",
    ...
    await emit_action(
        room, "phase_change", {"room_id": room_id, "to": "BIDDING", "from": "RESULT"}
    )
    await emit_state(room)
...
async def restart_game(sid, data: Dict[str, Any]):
    ...
    if not room.game or room.game.phase != "GAME_END":
        await sio.emit(
            "restart_rejected", {"message": "The game isn't over yet"}, to=sid
        )
        return
